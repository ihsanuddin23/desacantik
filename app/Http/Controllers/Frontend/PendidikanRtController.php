...
class PendidikanRtController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Hitung total laki-laki dan perempuan per RT
        $rtpendidikan = PendidikanRT::select(
            'identitasrt_id',
            DB::raw('SUM(laki_laki) as total_laki'),
            DB::raw('SUM(perempuan) as total_perempuan')
        )
            ->groupBy('identitasrt_id')
            ->get();
        ...
        return view("frontend2.pendidikanrt.index", compact("recentposts", "featuredposts", "categories", "pendidikanrt", "total_population", "identitasrt", "pendidikanRtData", "rtpendidikan"));
    }
}
